...
func TestExtractCount(t *testing.T) {
	...
	for _, test := range tests {
		result := utils.ExtractCount(test.input)
...
func TestExtractGenres(t *testing.T) {
	tests := []struct {
		...
		{"I love Sci-Fi and Fantasy movies", []string{"Horror", "Sci-Fi", "Fantasy"}},
		{"Mystery and Thriller are great", []string{"Horror", "Mystery", "Thriller"}},
		{"Animation movies are sugoi", []string{"Horror", "Animation"}},
		{"No genres mentioned", []string{"Horror"}},
		{"Sci-Fi is king", []string{"Horror", "Sci-Fi"}},
	}

	for _, test := range tests {
		result := utils.ExtractGenres(test.input)
...
func TestExtractDate(t *testing.T) {
	...
	for _, test := range tests {
		result := utils.ExtractDate(test.input)
