...
import (
	"nightmare_navigator/internal/movie_infos"
	"reflect"
	"strings"
	"testing"
)
...
func TestBuildMovieInfoString(t *testing.T) {
	tests := []struct {
		movieInfo movie_infos.MovieInfo
		expected  string
	}{
		{
			movieInfo: movie_infos.MovieInfo{
...
`,
	...
	for _, test := range tests {
		result := movie_infos.BuildMovieInfoString(test.movieInfo)
...
func TestBuildMovieInfoStrings(t *testing.T) {
	movieInfos := []movie_infos.MovieInfo{
		{
			Title:         "Event Horizon",
			OriginalTitle: "",
			IMDb:          "6.7",
...
`,
	...
	result := movie_infos.BuildMovieInfoStrings(movieInfos)
